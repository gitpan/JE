...
B<WARNING:> I might just rename this and call it C<keys>, since that seems
more intuitive.
...
below
4,294,967,295). It does not work on strings or String objects if the
property name is C<length>.

=item exists($property_name)

Returns a boolean indicating whether the property exists. Used by 
C<Object.prototype.hasOwnProperty>. (C<hasProperty> checks to see whether
...
This applies to object classes only (though this may change). It returns a 
