...
left alone (we assume you know what you are doing--but this will change
slightly when support is provided for binding Perl classes to JS). 
Otherwise the
...
ignored completely if C<value> is also given.

This hash ref calling convention is not supported by Arguments objects. It 
