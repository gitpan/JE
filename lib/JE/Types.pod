...
Every JS data type can be use as as string, boolean or number. It works
...
between this a future versions is to pass C<< recursive => 0 >> as
arguments.
...
The new value will be converted to a JS value
automatically if it is not one already. (See
L</UPGRADING VALUES>.) B<This is going to change.> For the sake of 
efficiency, C<prop> is going to stop upgrading automatically.
...
The arguments
are automatically upgraded B<but that is going to change>.
...
The arguments
are automatically upgraded B<but, again, that is going to change>.
...
B<To do:> Implement this method in JE's classes.
